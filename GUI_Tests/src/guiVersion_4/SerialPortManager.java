...
public class SerialPortManager implements SerialPortEventListener{
			...
			try {
				...
				if (charVal == START_BYTE && dataPacketFlag == false) { //Stores start byte
					...
					if (netText[1] != msgToChildren[0]) { //botID parent...						
						...
						/*if (!window.networkProtocol.isAlive()) {
							window.logAreaText.append("Re-starting: " +window.networkProtocol.getName()+ " \n");
							window.networkProtocol.start();
						}*/
					} 
					...
					/*if (NetworkProtocol.isSuccessPcktRxd()) {
						window.logAreaText.append("New package got in! " + "\n");
						window.networkProtocol.start();
					}*/ /*else {
						window.networkProtocol.interrupt();
					}*/
				}
	...
	public void sendData(int botID, int parent, int timeSlot, int packetSubTree){
		...
		/*//window.logAreaText.append("Sent -> ");
		System.out.print("Data Sent: [");
	...
	public void printChildMsg(int byteCounter){
		...
		//System.out.println("text: " + window.netAreaText.getText());
