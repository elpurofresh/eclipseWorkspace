	...
	/*public void parseChildMsg(int byteCounter){
		...
		for (int i = 1; i < byteCounter; i++) {
			...
			if (i == 1) {
				...
				packetParent = Integer.parseInt(new String(new byte[] {netText[i]})); //(int) netText[i] & 0xff;
			} else if (i == 3 || i == 4) {
				...
				packetSubTree = Integer.parseInt(new String(new byte[] {netText[i]}));//(int) netText[i] & 0xff;
			}
		}
