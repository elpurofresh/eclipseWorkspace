...
public class NetworkProtocol implements Runnable{
	...
	public static int[] msgToChildren 	= new int[MAX_NUM_PARAMS]; //{botID (0), myParent(1), myTimeSlot(2,3), hrs(4,5), min(6,7), sec(8,9)};
			...
			while (myTimeSlotCounter < myTimeSlotMax ) {//|| myParent == NULL <- not necessary in master
				...
				while (timeResult != 0) { 
					...
					try {
						...
						System.out.println("Interval-> " + window.networkThread.getName() + " 's status is: " + window.networkThread.getState());
						//e.printStackTrace();						
					}
				}
