	...
	public void serialEvent(SerialPortEvent spe) {
		...
		if (spe.getEventType() == SerialPortEvent.DATA_AVAILABLE) {//If one byte of data came in
			...
			try {
				...
				//System.out.println("charVal1: " + charVal);
				
				if (charVal != -1 && (System.currentTimeMillis() - startTime) > 100 ) { //To avoid "listening(saving)" to own's message.<<<<<<<<

					//main.gui.textInputArea.append(new String(new byte[] { (byte) charVal }));
					System.out.println("charVal2: " + charVal);
				...
				if (main.gui.isExpOneSelected()) {
					if (dataPacketFlag == false && charVal == START_BYTE) { //Stores start byte
						...
						main.gui.animation
								.updateMasterNodePic("Receiving Data...");
					}
				...
				/*if (main.gui.isExpOneSelected()) {
					...
					if (main.gui.isExpTwoSelected()) {
						rxExperTwo(charVal);
					}  
					if (main.gui.isExpThreeSelected()) {
